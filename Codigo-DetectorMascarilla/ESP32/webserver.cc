...
const char INDEX_HTML[] = 
	...
	"                    \n"
	"				     const iaResult = response.headers.get(\"X-IA-Result\") || \"Sin resultado\";\n"
	"                    videoElement.src = imageUrl;\n"
	"                    document.getElementById('ia_result').innerHTML = \"Resultado IA: <b>\" + iaResult + \"</b><br>Pulse de nuevo para stream.\";\n"
...
esp_err_t jpg_stream_httpd_handler(httpd_req_t *req){
    ...
    while(true){
        ...
        if(res == ESP_OK){  // Envia delimitador de inicio
            res = httpd_resp_send_chunk(req, _STREAM_BOUNDARY, strlen(_STREAM_BOUNDARY));
        }
        if(res == ESP_OK){  // Envia los METADATOS del frame (Tamaño y Content-Type del JPEG)
			size_t hlen = snprintf(part_buf, 64, _STREAM_PART, _jpg_buf_len);
            res = httpd_resp_send_chunk(req, part_buf, hlen);
        }
        if(res == ESP_OK){  // Envía los DATOS BINARIOS del JPEG
...
static char last_result_text[64] = "Sin procesar"; // <-- Global para almacenar texto IA
...
esp_err_t single_capture_handler(httpd_req_t *req) {
    ...
    int result =  GetModelPrediction();
    
     if (result == 1) {
        strcpy(last_result_text, "SIN cubrebocas");
    } else if (result == 0) {
        strcpy(last_result_text, "CON cubrebocas");
    } else {
        strcpy(last_result_text, "Error en inferencia");
    }
    ...
    // Devolver la imagen única y el resultado de la inferencia
    httpd_resp_set_type(req, "image/jpeg");
    httpd_resp_set_hdr(req, "X-IA-Result", last_result_text);
